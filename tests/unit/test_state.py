...
def test_state_store_tracks_recent_send_window(tmp_path: Path) -> None:
    ...
    assert store.was_sent_recently("NVDA", within_days=30, today=date(2026, 5, 8)) is True
    assert store.was_sent_recently("NVDA", within_days=30, today=date(2026, 6, 8)) is False
